...
SELECT SUM(price) FROM Books;";
			...
			//cmd.CommandText += " WHERE author=(SELECT author_id FROM Authors WHERE last_name='Cameron')";
			cmd.CommandText = select_string;
			...
			while (reader.Read())
			{
				...
				Console.WriteLine();
				totalPriceCameron += Convert.ToDouble(reader["price"]);
			}
			Console.WriteLine($"Общая стоимость всех книг:\t${totalPriceCameron}");
			Console.WriteLine();
